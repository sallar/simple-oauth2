...
module.exports = function(config) {
  ...
  function create(token) {
    if (this.token && 'refresh_token' in this.token) {
      token.refresh_token = this.token.refresh_token;
    }
    this.token = token;
    this.token.expires_at = this.token.expires_at ?
        (new Date(this.token.expires_at)) : (new Date).addSeconds(token.expires_in);
    return this;
  }
